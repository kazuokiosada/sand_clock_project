...
def get_arc_of_eclipse(a,b,θ1,θ2,step):
    """中心（0,0）の楕円上の弧を座標点列として求める
    a：横,b：縦,θ1：最初の角,θ2:最終の角,step：刻み"""
    t1=radians(θ1)
    t2=radians(θ2)
    st=radians(step)
    xylist=[]
    ###最初の座標
    xylist.append(get_xy(a,b,t1))
    ###後続の座標
    for t in np.arange(t1+st,t2+st,st):        
        ###角度tでの座標        
        xylist.append(get_xy(a,b,t))
    return xylist
def get_xy(a,b,t):
...
def get_another_cross_point(a,b,x1,y1,q):
    """a:横,b：縦,x1,y1:楕円上の点,q：(0,-q)：穴の頂点"""
